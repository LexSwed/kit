...
export function LexicalAutoEmbedPlugin<TEmbedConfig extends EmbedConfig>(
  ...
  const [activeEmbedConfig, setActiveEmbedConfig] = createSignal<TEmbedConfig | null>(null);
  ...
  const options = createMemo(() => {
    return activeEmbedConfig() != null && nodeKey() != null
      ? props.getMenuOptions(activeEmbedConfig()!, embedLinkViaActiveEmbedConfig, reset)
      : [];
  });

  const onSelectOption = (selectedOption: AutoEmbedOption, targetNode: TextNode | null, closeMenu: () => void) => {
    editor.update(() => {
